/*******************************************************************************
* nipa
* Updated 05/28/2011
* Linux port: replaced windows.h / TCHAR / Win32 APIs with POSIX equivalents.
*
* You're free to use this code in any way you want, as long as you provide
* credit in either the form of this source code or by name.
*******************************************************************************/

#define _GNU_SOURCE
#define _POSIX_C_SOURCE 200809L

#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <errno.h>
#include <limits.h>
#include <sys/stat.h>
#include <dirent.h>
#include <strings.h>
#include <zlib.h>

#include "nipa.h"

NPAHEAD NPAHead;
NPAENTRY *NPAEntry;

int offset = 0, subdir = 0, id = 0;
char origpath[PATH_MAX];

FILE *infile, *outfile;

char games[][20] = {
    "ChaosHead", "ChaosHeadTr1", "ChaosHeadTr2", "MuramasaTr", "Muramasa",
    "Sumaga", "Django", "DjangoTr", "Lamento", "LamentoTr", "sweetpool",
    "SumagaSP", "Demonbane", "MuramasaAD", "Axanael", "Kikokugai",
    "SonicomiTr2", "Sumaga3P", "Sonicomi", "LostX", "LostXTrailer",
    "DRAMAticalMurder", "Totono", "DRAMAticalMurderRC", "MuramasaSS", "\0"
};

unsigned char keytbl[][0x100] = {
    /* Chaos;Head Retail */
    ,

    /* Chaos;Head Trial 1 */
    ,

    /* Chaos;Head Trial 2 */
    ,

    /* Muramasa Trial */
    ,

    /* Muramasa Retail */
    ,

    /* Sumaga Retail */
    ,

    /* Zoku Satsuriku no Django Retail */
    ,

    /* Zoku Satsuriku no Django Trial */
    ,

    /* Lamento -Beyond the Void- Retail */
    ,

    /* Lamento -Beyond the Void- Trial */
    ,

    /* sweet pool Retail */
    ,

    /* Sumaga Special Retail */
    ,

    /* Demonbane The Best */
    ,

    /* MuramasaAD */
    ,

    /* Axanael + Trial */
    ,

    /* Kikokugai */
    ,

    /* Sonicomi Trial 2 */
    ,

    /* Sumaga 3% Trial */
    ,

    /* Sonicomi Version 1.0 */
    ,

    /* Guilty Crown Lost Xmas */
    ,

    /* Guilty Crown Lost Xmas Trailer */
    ,

    /* DRAMAtical Murder */
    ,

    /* Totono */
    ,

    /* DRAMAtical Murder re:connect */
    ,

    /* MuramasaSS */
    ,

    0
};

enum { MODE_HELP, MODE_EXTRACT, MODE_CREATE };

int main(int argc, char **argv)
{
    int i = 0, encryption = 0, mode = MODE_HELP;

    printf("nipa\n\n");

    if (argc > 1 && strlen(argv[1]) > 1 && argv[1][i++] == '-')
    {
        while (argv[1][i] != '\0')
        {
            switch (argv[1][i++])
            {
            case 'x':
                if (argc < 3)
                {
                    printf("Invalid arguments for -x\n");
                    return 1;
                }
                mode = MODE_EXTRACT;
                break;
            case 'c':
                if (encryption)
                {
                    if (argc < 5)
                    {
                        printf("Invalid arguments for -gc\n");
                        return 1;
                    }
                }
                else
                {
                    if (argc < 4)
                    {
                        printf("Invalid arguments for -c\n");
                        return 1;
                    }
                }
                mode = MODE_CREATE;
                break;
            case 'h':
                mode = MODE_HELP;
                break;
            case 'z':
                NPAHead.compress = 1;
                break;
            case 'g':
                if (mode == MODE_CREATE)
                {
                    if (argc < 5)
                    {
                        printf("Invalid arguments for -cg\n");
                        return 1;
                    }
                }
                else
                {
                    if (argc < 4)
                    {
                        printf("Invalid arguments for -g\n");
                        return 1;
                    }
                }
                encryption = 1;
                break;
            default:
                printf("Unknown argument '%c', ignoring.\n", argv[1][i-1]);
                break;
            }
        }
    }

    if (mode == MODE_HELP)
    {
        printf(
            "usage:\n"
            "General\n"
            "\t-h - Help. Displays this information.\n\n"
            "Extraction\n"
            "\t-x (file) - Extract NPA archive\n"
            "\t-g (id) - Game ID for encryption (if applicable). Defaults to ChaosHead if no encryption is entered\n\n"
            "Creation\n"
            "\t-c (folder) (output file) - Create archive\n"
            "\t-z - Compress files\n\n"
            "Examples\n"
            "\t-x nss.npa - Extract nss.npa into the folder \"nss\"\n"
            "\t-xg nss.npa MuramasaTr - Extract nss.npa into the folder \"nss\" using the Muramasa Trial encryption\n"
            "\t-c nss nss.npa - Create an archive out of the folder \"nss\"\n"
            "\t-cz nss nss.npa - Create a compressed archive out of the folder \"nss\"\n\n"
            "Maker - Name - ID\n"
            "Nitro+ - Chaos;Head - ChaosHead\n"
            "Nitro+ - Chaos;Head Trial 1 - ChaosHeadTr1\n"
            "Nitro+ - Chaos;Head Trial 2 - ChaosHeadTr2\n"
            "Nitro+ - Demonbane The Best - Demonbane\n"
            "Nitro+ - FullMetalDaemon MURAMASA - Muramasa\n"
            "Nitro+ - FullMetalDaemon MURAMASA Janen Hen - MuramasaAD\n"
            "Nitro+ - FullMetalDaemon MURAMASA Trial - MuramasaTr\n"
            "Nitro+ - FullMetalDaemon MURAMASA Shokuzai hen - MuramasaSS\n"
            "Nitro+ - Guilty Crown Lost Xmas - LostX\n"
            "Nitro+ - Guilty Crown Lost Xmas Trailer - LostXTrailer\n"
            "Nitro+ - Kikokugai - Kikokugai\n"
            "Nitro+ - Sonicomi - Sonicomi\n"
            "Nitro+ - Sonicomi Trial 2 - SonicomiTr2\n"
            "Nitro+ - Sumaga - Sumaga\n"
            "Nitro+ - Sumaga3P - Sumaga 3%% Trial\n"
            "Nitro+ - Sumaga Special - SumagaSP\n"
            "Nitro+ - Kimi to Kajono to Kanojo no Koi - Totono\n"
            "Nitro+ - Axanael - Axanael\n"
            "Nitro+ - Zoku Satsuriku no Django - Django\n"
            "Nitro+ - Zoku Satsuriku no Django Trial - DjangoTr *NOT WORKING*\n"
            "Nitro+ ChiRAL - Lamento -Beyond the Void- - Lamento\n"
            "Nitro+ ChiRAL - Lamento -Beyond the Void- Trial - LamentoTr *UNKNOWN*\n"
            "Nitro+ ChiRAL - sweet pool - sweetpool\n"
            "Nitro+ ChiRAL - Dramatical Murder - DramaticalMurder\n"
            "Nitro+ ChiRAL - DRAMAtical Murder re:connect - DramaticalMurderRC\n"
        );
        return 0;
    }

    if (encryption == 1)
    {
        for (i = 0; games[i][0] != '\0' && strcasecmp(games[i], argv[argc-1]) != 0; i++);

        if (games[i][0] == '\0')
        {
            printf("Unsupported game ID: %s\n", argv[argc-1]);
            return 0;
        }
        else
        {
            NPAHead.gameid = i;
        }
    }

    if (mode == MODE_EXTRACT)
    {
        parsenpa(argv[2], encryption);
    }
    else if (mode == MODE_CREATE)
    {
        createnpa(argc-2, argv+2, encryption);
    }

    return 0;
}

/*
* Header decryption key algorithm. Symmetric.
*/
int my_crypt(int curnum, int curfile)
{
    int key = 0xFC * curnum;
    int temp = 0;

    if ((NPAHead.gameid == LAMENTO && NPAHead.encrypt) || (NPAHead.gameid == LAMENTOTR && NPAHead.encrypt))
        temp = NPAHead.key1 + NPAHead.key2;
    else
        temp = NPAHead.key1 * NPAHead.key2;

    key -= temp >> 0x18;
    key -= temp >> 0x10;
    key -= temp >> 0x08;
    key -= temp  & 0xff;

    key -= curfile >> 0x18;
    key -= curfile >> 0x10;
    key -= curfile >> 0x08;
    key -= curfile;

    return key & 0xff;
}

/*
* File data decryption key algorithm.
*/
int my_crypt2(int curnum, char *name)
{
    int i = 0;
    int key1 = 0;
    int key2 = NPAHead.key1 * NPAHead.key2;
    int key = 0;

    switch (NPAHead.gameid)
    {
        case AXANAEL:
        case KIKOKUGAI:
        case SONICOMITR2:
        case SONICOMI:
        case LOSTX:
        case DRAMATICALMURDER:
        case DRAMATICALMURDERRC:
        case MURAMASASS:
            key1 = 0x20101118;
            break;
        case TOTONO:
            key1 = 0x12345678;
            break;
        default:
            key1 = 0x87654321;
            break;
    }

    for (i = 0; name[i] != 0; i++)
        key1 -= name[i];

    key = key1 * i;

    if (NPAHead.gameid != LAMENTO && NPAHead.gameid != LAMENTOTR)
    {
        key += key2;
        key *= NPAEntry[curnum].origsize;
    }

    return key & 0xff;
}

void parsenpa(char *input, int encryption)
{
    int i = 0, x = 0;

    infile = fopen(input, "rb");
    if (!infile)
    {
        printf("Could not open %s\n", input);
        exit(1);
    }

    for (i = 0; input[i] != '.' && input[i] != '\0'; i++);
    input[i] = 0x00;

    /* Create a directory based on the input file's name then use it for output */
    mkdir(input, 0755);
    if (chdir(input) != 0)
    {
        printf("Could not chdir to %s\n", input);
        exit(1);
    }

    fread(NPAHead.head, 1, 7, infile);
    if (strncmp("NPA\x01", NPAHead.head, 4) == 0)
    {
        printf("Parsing NPA...\n");

        fread(&NPAHead.key1,      1, 4, infile);
        fread(&NPAHead.key2,      1, 4, infile);
        fread(&NPAHead.compress,  1, 1, infile);
        fread(&NPAHead.encrypt,   1, 1, infile);
        fread(&NPAHead.totalcount,1, 4, infile);
        fread(&NPAHead.foldercount,1,4, infile);
        fread(&NPAHead.filecount, 1, 4, infile);
        fread(&NPAHead.null,      1, 8, infile);
        fread(&NPAHead.start,     1, 4, infile);

        if (NPAHead.encrypt == 1 && encryption == 0)
        {
            printf("This is an encrypted archive. Please read the help information and select an encryption.\n");
            exit(1);
        }

        NPAEntry = calloc(NPAHead.totalcount, sizeof(*NPAEntry));

        for (i = 0; i < NPAHead.totalcount; i++)
        {
            fread(&NPAEntry[i].nlength, 1, 4, infile);

            NPAEntry[i].filename = (char*)calloc(NPAEntry[i].nlength + 1, sizeof(char));
            fread(NPAEntry[i].filename, 1, NPAEntry[i].nlength, infile);

            for (x = 0; x < NPAEntry[i].nlength; x++)
                NPAEntry[i].filename[x] += my_crypt(x, i);

            NPAEntry[i].filename[x] = '\0';

            fread(&NPAEntry[i].type,    1, 1, infile);
            fread(&NPAEntry[i].fileid,  1, 4, infile);
            fread(&NPAEntry[i].offset,  1, 4, infile);
            fread(&NPAEntry[i].compsize,1, 4, infile);
            fread(&NPAEntry[i].origsize,1, 4, infile);

            printf("%04d: %-50s 0x%08X [%08X]\n", i, NPAEntry[i].filename,
                   NPAEntry[i].offset, NPAEntry[i].compsize);

            if (NPAEntry[i].type == 1)
            {
                mkdir(NPAEntry[i].filename, 0755);
            }
            else
            {
                extractnpa(i, ftell(infile), NPAEntry[i].filename);
            }
        }
    }
    else
    {
        printf("Invalid NPA archive\n");
    }
}

void extractnpa(int i, int pos, char *destination)
{
    unsigned char *buffer = (unsigned char*)calloc(NPAEntry[i].compsize, sizeof(char));
    FILE *out = NULL;

    if (!buffer)
    {
        printf("Could not allocate %d bytes of memory for buffer\n", NPAEntry[i].compsize);
        exit(1);
    }

    out = fopen(destination, "wb");
    if (!out)
    {
        printf("--WARNING: Cannot write file. %s\n", strerror(errno));
        free(buffer);
        return;
    }

    fseek(infile, NPAEntry[i].offset + NPAHead.start + 0x29, SEEK_SET);
    fread(buffer, 1, NPAEntry[i].compsize, infile);

    if (NPAHead.encrypt == 1)
    {
        int key = my_crypt2(i, NPAEntry[i].filename);
        int x = 0;
        int len = 0x1000;

        if (NPAHead.gameid != LAMENTO && NPAHead.gameid != LAMENTOTR)
            len += strlen(NPAEntry[i].filename);

        for (x = 0; x < NPAEntry[i].compsize && x < len; x++)
        {
            if (NPAHead.gameid == LAMENTO || NPAHead.gameid == LAMENTOTR)
            {
                buffer[x] = keytbl[NPAHead.gameid][buffer[x]] - key;
            }
            else if (NPAHead.gameid == TOTONO)
            {
                unsigned char r = buffer[x];
                char r2;
                r = keytbl[NPAHead.gameid][r];
                r = keytbl[NPAHead.gameid][r];
                r = keytbl[NPAHead.gameid][r];
                r = ~r;
                r2 = (char)r - key - x;
                buffer[x] = (unsigned char)r2;
            }
            else
            {
                buffer[x] = (keytbl[NPAHead.gameid][buffer[x]] - key) - x;
            }
        }
    }

    if (NPAHead.compress == 1)
    {
        char *zbuffer = (char*)calloc(NPAEntry[i].origsize, sizeof(char));
        uLongf origsize = (uLongf)NPAEntry[i].origsize;

        if (uncompress((Bytef*)zbuffer, &origsize,
                       (const Bytef*)buffer, (uLong)NPAEntry[i].compsize) != Z_OK)
        {
            printf("Uncompress failed!\n");
            exit(1);
        }
        NPAEntry[i].origsize = (int)origsize;
        fwrite(zbuffer, 1, NPAEntry[i].origsize, out);
        free(zbuffer);
    }
    else
    {
        fwrite(buffer, 1, NPAEntry[i].origsize, out);
    }

    free(buffer);
    fclose(out);

    fseek(infile, pos, SEEK_SET);
}

void createnpa(int count, char **inarr, int encrypt)
{
    int i = 0, x = 0;
    char encname[PATH_MAX];
    unsigned char encTable[0x100];

    encname;
    count;
    strcat(origpath, inarr[0]);
    /* On Linux we use a POSIX glob-style path; parsedir handles the actual iteration */
    strcat(origpath, "/");

    printf("Parsing directory structure, this could take a moment...\n");
    parsedir(origpath);

    /* Prepare the struct */
    strcat(NPAHead.head, "NPA\x01\x00\x00\x00");

    NPAHead.key1    = 0x4147414E;
    NPAHead.key2    = 0x21214f54;
    NPAHead.encrypt = encrypt;
    if (encrypt)
    {
        size_t k;
        for (k = 0; k < sizeof(encTable)/sizeof(encTable[0]); ++k)
            encTable[keytbl[NPAHead.gameid][k]] = k;
    }
    NPAHead.filecount  = NPAHead.totalcount - NPAHead.foldercount;
    NPAHead.start     += NPAHead.totalcount * 0x15;
    NPAEntry[0].offset = 0;

    outfile = fopen(inarr[1], "wb");
    if (!outfile)
    {
        printf("Could not open %s for writing\n", inarr[1]);
        exit(1);
    }

    /* Write the header */
    fwrite(NPAHead.head,        1, 7, outfile);
    fwrite(&NPAHead.key1,       1, 4, outfile);
    fwrite(&NPAHead.key2,       1, 4, outfile);
    fwrite(&NPAHead.compress,   1, 1, outfile);
    fwrite(&NPAHead.encrypt,    1, 1, outfile);
    fwrite(&NPAHead.totalcount, 1, 4, outfile);
    fwrite(&NPAHead.foldercount,1, 4, outfile);
    fwrite(&NPAHead.filecount,  1, 4, outfile);
    fwrite("\x00\x23\x65\x6E\x73\x75\x65\x00", 1, 8, outfile);
    fwrite(&NPAHead.start,      1, 4, outfile);

    /* Write the file table */
    for (i = 0; i < NPAHead.totalcount; i++)
    {
        for (x = 0; x < NPAEntry[i].nlength; x++)
            encname[x] = NPAEntry[i].filename[x] - my_crypt(x, i);

        fwrite(&NPAEntry[i].nlength, 1, 4, outfile);
        fwrite(encname,              1, NPAEntry[i].nlength, outfile);
        fwrite(&NPAEntry[i].type,    1, 1, outfile);
        fwrite(&NPAEntry[i].fileid,  1, 4, outfile);
        fwrite(&NPAEntry[i].offset,  1, 4, outfile);
        fwrite(&NPAEntry[i].compsize,1, 4, outfile);
        fwrite(&NPAEntry[i].origsize,1, 4, outfile);
    }

    {
        int bufferSz = 0, zbufferSz = 0;
        Byte *buffer = 0, *zbuffer = 0;
        const int origpathLen = strlen(origpath);

        for (i = 0; i < NPAHead.totalcount; i++)
        {
            if (NPAEntry[i].type == 2) /* File */
            {
                char tempstr[PATH_MAX];
                tempstr;
                FILE *fin;

                strcpy(tempstr, origpath);
                strncat(tempstr, NPAEntry[i].filename, PATH_MAX - origpathLen - 1);

                printf("%04d: %-50s 0x%08X [%08X]\n", i,
                       NPAEntry[i].filename, NPAEntry[i].offset, NPAEntry[i].origsize);

                fin = fopen(tempstr, "rb");
                if (fin)
                {
                    Byte *writeSrc = 0;
                    int writeSrcBytes = 0;

                    if (bufferSz < NPAEntry[i].origsize)
                    {
                        free(buffer);
                        buffer = (Byte*)calloc(NPAEntry[i].origsize, sizeof(Byte));
                        bufferSz = NPAEntry[i].origsize;
                    }

                    fread(buffer, 1, NPAEntry[i].origsize, fin);

                    if (NPAHead.compress == 1)
                    {
                        if (zbufferSz < NPAEntry[i].origsize)
                        {
                            free(zbuffer);
                            zbuffer = (Byte*)calloc(NPAEntry[i].origsize, sizeof(Byte));
                            zbufferSz = NPAEntry[i].origsize;
                        }
                        {
                            uLongf compsize = (uLongf)NPAEntry[i].origsize;
                            compress(zbuffer, &compsize, buffer, NPAEntry[i].origsize);
                            NPAEntry[i].compsize = (int)compsize;
                        }
                        writeSrc = zbuffer;
                        writeSrcBytes = NPAEntry[i].compsize;
                    }
                    else
                    {
                        writeSrc = buffer;
                        writeSrcBytes = NPAEntry[i].origsize;
                    }

                    if (NPAHead.encrypt)
                    {
                        int key = my_crypt2(i, NPAEntry[i].filename);
                        int lx = 0;
                        int len = 0x1000;

                        if (NPAHead.gameid != LAMENTO && NPAHead.gameid != LAMENTOTR)
                            len += strlen(NPAEntry[i].filename);

                        for (lx = 0; lx < writeSrcBytes && lx < len; lx++)
                        {
                            if (NPAHead.gameid == LAMENTO || NPAHead.gameid == LAMENTOTR)
                            {
                                writeSrc[lx] = encTable[(writeSrc[lx] + key) & 0xFF];
                            }
                            else if (NPAHead.gameid == TOTONO)
                            {
                                char r = writeSrc[lx] + lx + key;
                                unsigned char r2;
                                r = ~r;
                                r2 = (unsigned char)r;
                                r2 = encTable[r2];
                                r2 = encTable[r2];
                                r2 = encTable[r2];
                                writeSrc[lx] = r2;
                            }
                            else
                            {
                                writeSrc[lx] = encTable[(writeSrc[lx] + lx + key) & 0xFF];
                            }
                        }
                    }

                    fwrite(writeSrc, 1, writeSrcBytes, outfile);
                    fclose(fin);
                }
                else
                {
                    printf("Could not open %s\n", tempstr);
                }
            }
        }
        free(buffer);
        free(zbuffer);
    }

    /* Fix the header for compressed files */
    if (NPAHead.compress == 1)
    {
        fseek(outfile, 0x29, SEEK_SET);

        for (i = 0; i < NPAHead.totalcount; i++)
        {
            if (i != 0)
                NPAEntry[i].offset = NPAEntry[i-1].offset + NPAEntry[i-1].compsize;

            fseek(outfile, NPAEntry[i].nlength + 9, SEEK_CUR);
            fwrite(&NPAEntry[i].offset,  1, 4, outfile);
            fwrite(&NPAEntry[i].compsize,1, 4, outfile);
            fwrite(&NPAEntry[i].origsize,1, 4, outfile);
        }
    }

    fclose(outfile);
}

void addentry(char *path, char *name, int eid, int type, int sd)
{
    int i = 0;
    FILE *tempf;
    int pathLen = strlen(path);
    char *temp2 = (char*)calloc(pathLen + strlen(name) + 1, sizeof(char));
    const char *relativeDir;
    int relativePathSz;

    NPAEntry = realloc(NPAEntry, (NPAHead.totalcount + 1) * 0x1c);
    NPAEntry[NPAHead.totalcount].compsize = 0;
    NPAEntry[NPAHead.totalcount].origsize = 0;

    if (path[pathLen-1] == '*')
    {
        path[pathLen-1] = 0x00;
        --pathLen;
    }

    relativeDir = path + strlen(origpath);
    relativePathSz = strlen(relativeDir) + strlen(name) + 1;

    NPAEntry[NPAHead.totalcount].filename = (char*)calloc(relativePathSz, sizeof(char));
    strcat(NPAEntry[NPAHead.totalcount].filename, relativeDir);
    strcat(NPAEntry[NPAHead.totalcount].filename, name);

    strcpy(temp2, path);
    strcat(temp2, name);

    NPAEntry[NPAHead.totalcount].nlength = strlen(NPAEntry[NPAHead.totalcount].filename);
    NPAHead.start += NPAEntry[NPAHead.totalcount].nlength;
    NPAEntry[NPAHead.totalcount].fileid = eid;

    if (type != 0)
        NPAEntry[NPAHead.totalcount].type = 1;
    else
        NPAEntry[NPAHead.totalcount].type = 2;

    if (NPAEntry[NPAHead.totalcount].type == 2)
    {
        tempf = fopen(temp2, "rb");
        if (!tempf)
        {
            printf("Could not open %s\n", temp2);
            exit(1);
        }
        fseek(tempf, 0, SEEK_END);
        NPAEntry[NPAHead.totalcount].compsize = ftell(tempf);
        NPAEntry[NPAHead.totalcount].origsize = NPAEntry[NPAHead.totalcount].compsize;
        fclose(tempf);
    }

    if (NPAHead.totalcount > 0)
    {
        offset += NPAEntry[NPAHead.totalcount-1].compsize;
        NPAEntry[NPAHead.totalcount].offset = offset;
    }

    if (sd > 1)
        NPAEntry[NPAHead.totalcount].fileid = ++id;

    NPAHead.totalcount++;
    free(temp2);
    (void)i;
}

/* Replaces the Win32 FindFirstFile / FindNextFile directory walker */
void parsedir(char *path)
{
    DIR *dirp;
    struct dirent *dp;
    struct stat st;
    char dirpath[PATH_MAX];
    int curid = id++;

    /* Strip trailing '*' or '/' to get a clean dir path for opendir */
    strncpy(dirpath, path, PATH_MAX - 1);
    dirpath[PATH_MAX - 1] = '\0';
    {
        int len = strlen(dirpath);
        if (len > 0 && (dirpath[len-1] == '*' || dirpath[len-1] == '/'))
            dirpath[len-1] = '\0';
    }

    dirp = opendir(dirpath);
    if (!dirp)
    {
        printf("Could not open directory: %s\n", dirpath);
        return;
    }

    while ((dp = readdir(dirp)) != NULL)
    {
        char fullpath[PATH_MAX];
        int is_dir;

        fullpath;
        dirpath;
        /* Skip . and .. */
        if (strcmp(dp->d_name, ".") == 0 || strcmp(dp->d_name, "..") == 0)
            continue;

        snprintf(fullpath, sizeof(fullpath), "%s/%s", dirpath, dp->d_name);
        if (stat(fullpath, &st) != 0)
            continue;

        is_dir = S_ISDIR(st.st_mode);

        if (is_dir)
            subdir++;

        addentry(path, dp->d_name, curid, is_dir, subdir);

        if (is_dir)
        {
            char *subpath = (char*)calloc(PATH_MAX, sizeof(char));
            snprintf(subpath, PATH_MAX, "%s/%s/", dirpath, dp->d_name);
            NPAHead.foldercount++;
            parsedir(subpath);
            subdir--;
            free(subpath);
        }
    }
    closedir(dirp);
}
